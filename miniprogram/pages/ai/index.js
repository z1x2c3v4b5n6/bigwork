...
const fallbackCourseRecommendations = [
  {
    title: '英语写作冲刺训练营',
    teacher: '王老师',
...
const fallbackMaterialRecommendations = [
  {
    title: '英语作文万能句型速查表',
    type: '资料',
    url: '',
...
Page({
  ...
  async loadOverview() {
    ...
    try {
      const response = await apiRequest({ path: '/ai/overview' });
      ...
      const recommendedCourses =
        Array.isArray(response?.recommendedCourses) && response.recommendedCourses.length
      ...
      const message =
        error?.statusCode === 401
          ...
          : error?.message || '获取 AI 助手概览失败，请稍后再试。';
