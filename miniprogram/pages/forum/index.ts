...
const mapTopic = (topic: Record<string, unknown>): ForumTopicView => ({
  ...
  likedByUser: Boolean(topic.likedByMe ?? topic.likedByUser ?? false),
  ...
  tags: Array.isArray(topic.tags)
    ? (topic.tags as unknown[])
        .map((tag) => String(tag).trim())
        .filter((value) => value.length > 0)
    : [],
  replyCount: Number(topic.replies ?? topic.replyCount ?? 0),
});

const mapPost = (post: Record<string, unknown>): ForumPostView => ({
...
const resolveLikeButtonText = (topic: ForumTopicView | null) => (topic?.likedByUser ? '取消点赞' : '点赞');

Page({
  data: {
    topics: [] as ForumTopicView[],
    selectedTopicId: '',
    activeTopic: null as ForumTopicView | null,
  ...
  async loadTopics() {
    ...
    try {
      ...
      this.setData({ errorMessage: apiError?.message || '加载考研论坛失败，请稍后重试。' });
    } finally {
  ...
  async toggleLike() {
    ...
    try {
      await ensureSession();
      const response = await apiRequest<{ likes: number; liked: boolean }>({
      ...
      const topics = this.data.topics.map((topic) =>
        topic.id === topicId
          ? { ...topic, likes: response.likes, likedByUser: response.liked }
          : topic,
      );
