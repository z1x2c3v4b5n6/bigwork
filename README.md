...
- **考研论坛**：`/forum` 页面提供话题与帖子增删，支持点赞、回复删除，并可在后台进行话题、帖子审核与删除。
...
> 📄 仓库内提供 `server/schema/structure.sql`，可以直接在 Navicat 中执行，快速创建 `users`、`forum_topics`、`forum_posts`、`forum_topic_likes` 等基础表并写入示例数据。
